...
if (!_notvehicle) then {
	{
		if (!(_x isEqualTo _target) && {(_x isKindOf "Car" or _x isKindOf "Air" or _x isKindOf "B_CargoNet_01_ammo_F" or _x isKindOf "Ship" or _x isKindOf "Tank" or _x isKindOf "B_Slingload_01_Cargo_F") && !(side _x isEqualTo west) && !({alive _x} count crew _x > 0)}) then {_objects pushback _x};
