...
private _doSalvage = {
  [_this] spawn {
    ...
    for "_x" from 0 to (_steel - 1) do {
        if(!(_veh isKindOf "Car" || _veh isKindOf "B_CargoNet_01_ammo_F") && !(_veh canAdd "OT_Steel")) exitWith {
            "Vehicle is full, use a truck || ammobox for more storage" call OT_fnc_notifyMinor;
        };
        _done = _done + 1;
        _veh addItemCargoGlobal ["OT_Steel", 1];
    };
    if(_plastic > 0 && ((_veh isKindOf "Car" || _veh isKindOf "B_CargoNet_01_ammo_F") || (_veh canAdd "OT_Plastic"))) then {
...
private _objects = player nearEntities [["Car","B_CargoNet_01_ammo_F","Air","Ship"],20];
_filtered = [];
{
