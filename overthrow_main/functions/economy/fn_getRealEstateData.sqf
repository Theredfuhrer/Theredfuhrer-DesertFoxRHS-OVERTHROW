...
([_type] call {
	...
	if(_type in OT_lowPopHouses) exitWith {[3000,8]};
	if(_type in OT_mansions) exitWith {[50000,10]};
	if(_type in OT_medPopHouses) exitWith {[7500,12,0.2]};
	...
	[]
}) params [["_baseprice", 1000],["_totaloccupants",4],["_multiplier",0.35]];
...
private _lease = round((_stability * _population) * ((_baseprice * _multiplier) * _totaloccupants * 0.2));
